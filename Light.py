...
class Light:
    ...
    LIGHT_SENSOR = Pin(6, Pin.IN, Pin.PULL_UP)             #INPUT pin

    def __init__(self) -> None:
        pass
    
    def getLight(self, httpResJson):
        if self.LIGHT_SENSOR.value()==1:
            print("Helligkeit: dunkel")
            self.lcd.clear()
            self.lcd.putstr(str(httpResJson["name"]) + ": Helligkeit: dunkel")
            time.sleep(5)
            return "false"
        else:
            print("Helligkeit: hell")
            self.lcd.clear()
            self.lcd.putstr(str(httpResJson["name"]) + ": Helligkeit: hell")
            time.sleep(5)
            return "true"
